  ...
  return (
    <div className="bg-white dark:bg-night-800 rounded-xl shadow-lg backdrop-filter backdrop-blur-xl h-full flex flex-col">
      ...
      <div className={`flex-1 ${showScroll ? 'overflow-y-auto' : ''}`}>
        ...
        {/* Workflow Indicator for Project Completion Status */}
        <div className="mx-6 mb-4 bg-gradient-to-r from-bioluminescent-50/80 to-white/70 dark:from-bioluminescent-900/20 dark:to-gray-800/60 rounded-lg p-4 transition-all duration-300" style={{
          border: 'none',
          borderColor: 'rgba(0, 247, 255, 0.7)',
          boxShadow: '0 0 0 1px rgba(0, 247, 255, 0.4), 0 8px 25px rgba(0, 247, 255, 0.18), inset 0 1px 0 rgba(255, 255, 255, 0.3)',
          // Dark mode override
          ...(document.body.classList.contains('dark') && {
            background: 'linear-gradient(135deg, rgba(0, 247, 255, 0.1), rgba(31, 41, 55, 0.8))',
            boxShadow: '0 0 0 1px rgba(0, 247, 255, 0.3), 0 8px 25px rgba(0, 247, 255, 0.15)'
          })
        }}>
          <h4 className="text-sm font-semibold text-bioluminescent-700 dark:text-bioluminescent-300 mb-3 flex items-center gap-2">
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div className="flex items-center space-x-4 text-xs">
            ...
            <div className={`flex items-center gap-2 px-3 py-2 rounded-lg transition-all ${
              hasAdequateProjectInfo() ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-200' :
              'bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-200'
            }`}>
              <div className={`w-2 h-2 rounded-full ${
                hasAdequateProjectInfo() ? 'bg-green-600' : 'bg-blue-600'
              }`}></div>
              <span className="font-medium">1. Project Info</span>
              {hasAdequateProjectInfo() && (
            ...
            {/* Step 2: Save Project */}
            <div className={`flex items-center gap-2 px-3 py-2 rounded-lg transition-all ${
              isProjectSaved() ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-200' :
              hasAdequateProjectInfo() ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-200' :
              'bg-gray-100 dark:bg-gray-700/30 text-gray-600 dark:text-gray-400'
            }`}>
              <div className={`w-2 h-2 rounded-full ${
                isProjectSaved() ? 'bg-green-600' : 
                hasAdequateProjectInfo() ? 'bg-blue-600' : 'bg-gray-400'
              }`}></div>
              <span className="font-medium">2. Save Project</span>
              {isProjectSaved() && (
                <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                </svg>
            ...
            {/* Step 3: Select Parent Folder */}
            <div className={`flex items-center gap-2 px-3 py-2 rounded-lg transition-all ${
              displayData.project_path ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-200' :
              isProjectSaved() ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-200' :
              'bg-gray-100 dark:bg-gray-700/30 text-gray-600 dark:text-gray-400'
            }`}>
              <div className={`w-2 h-2 rounded-full ${
                displayData.project_path ? 'bg-green-600' : 
                isProjectSaved() ? 'bg-blue-600' : 'bg-gray-400'
              }`}></div>
              <span className="font-medium">3. Select Parent Folder</span>
              {displayData.project_path && (
                <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                </svg>
            ...
            <div className={`flex items-center gap-2 px-3 py-2 rounded-lg transition-all ${
              displayData.folder_created ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-200' :
              (isProjectSaved() && hasAdequateProjectInfo() && displayData.project_path) ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-200' :
              'bg-gray-100 dark:bg-gray-700/30 text-gray-600 dark:text-gray-400'
            }`}>
              <div className={`w-2 h-2 rounded-full ${
                displayData.folder_created ? 'bg-green-600' : 
                (isProjectSaved() && hasAdequateProjectInfo() && displayData.project_path) ? 'bg-blue-600' : 'bg-gray-400'
              }`}></div>
              <span className="font-medium">4. Create Structure</span>
              {displayData.folder_created && (
                <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                </svg>
          ...
          {/* Current suggested project folder name */}
          {isProjectSaved() && hasAdequateProjectInfo() && !displayData.folder_created && (
            <div className="mt-3 p-3 bg-white/60 dark:bg-gray-800/50 rounded-lg backdrop-filter backdrop-blur-sm">
              <div className="text-xs text-gray-600 dark:text-gray-400 mb-1">Suggested project folder name:</div>
              <div className="font-mono text-sm text-gray-900 dark:text-gray-100">
                {generateSuggestedPath().split(/[\\//]/).pop()}
              </div>
            </div>
          )}
        </div>
